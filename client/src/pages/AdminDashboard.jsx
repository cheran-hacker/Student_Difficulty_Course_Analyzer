...
const AdminDashboard = () => {
    ...
    const [accessDept, setAccessDept] = useState('');
    ...
    const handleBulkToggle = async (status) => {
        const filteredUsers = [...students, ...facultyList].filter(u => {
            ...
            const matchesDept = !accessDept || u.department === accessDept;
            const matchesYear = !accessYear || u.year === accessYear;
                    ...
                    {activeTab === 'settings' && (
                        ...
                        >
                            <div className="bg-white dark:bg-gray-800/40 backdrop-blur-md p-8 rounded-3xl border border-gray-100 dark:border-gray-700/50 shadow-sm">
                                ...
                                <div className="flex flex-col gap-4 mb-8 pb-8 border-b border-gray-100 dark:border-white/5">
                                    {/* Row 1: Role tabs + bulk actions */}
                                    <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                                        <div className="flex p-1.5 bg-gray-100 dark:bg-gray-900/50 rounded-2xl w-full md:w-auto">
                                            {['all', 'student', 'faculty'].map(tab => (
                                                <button
                                                    key={tab}
                                                    onClick={() => { setAccessFilter(tab); setAccessYear(''); }}
                                                ...
                                                >
                                                    {tab === 'all' ? 'All Roles' : tab === 'faculty' ? 'Facultys' : 'Students'}
                                                </button>
                                            ))}
                                        </div>
                                    ...
                                    {/* Row 2: Search + Dept + Year filters */}
                                    <div className="flex flex-col sm:flex-row gap-3">
                                        <div className="relative flex-1">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-10 pr-4 py-3.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-white/5 rounded-2xl text-sm focus:ring-2 focus:ring-blue-500/30 transition-all outline-none"
                                                value={userSearch}
                                        ...
                                        <select
                                            value={accessDept}
                                            onChange={(e) => setAccessDept(e.target.value)}
                                            className="px-4 py-3.5 bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-white/5 rounded-2xl text-sm font-bold text-gray-700 dark:text-gray-300 outline-none focus:ring-2 focus:ring-blue-500/30 transition-all"
                                        >
                                            <option value="">All Departments</option>
                                            {DEPARTMENTS.map(dept => (
                                                <option key={dept} value={dept}>{dept}</option>
                                        ...
                                        {(accessFilter === 'all' || accessFilter === 'student') && (
                                            <select
                                                value={accessYear}
                                        ...
                                        {(accessDept || accessYear || userSearch) && (
                                            <button
                                                onClick={() => { setAccessDept(''); setAccessYear(''); setUserSearch(''); }}
                                                className="px-4 py-3.5 rounded-2xl text-xs font-black uppercase tracking-widest text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 border border-red-500/20 transition-all whitespace-nowrap"
                                            >
                                                Clear
                                            </button>
                                        )}
                                    ...
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                                            {[...students, ...facultyList]
                                                .filter(u => {
                                                    const q = userSearch.toLowerCase();
                                                    return (accessFilter === 'all' || u.role === accessFilter) &&
                                                        (!accessDept || u.department === accessDept) &&
                                                        (!accessYear || u.year === accessYear) &&
                                                    ...
                                                    <tr key={user._id} className="group hover:bg-gray-50/50 dark:hover:bg-white/5 transition-all">
                                                        <td className="px-4 py-5">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <div>
                                                                    <div className="text-sm font-bold text-gray-900 dark:text-white leading-none mb-0.5">{user.name}</div>
                                                                    <div className="text-[10px] text-gray-400 font-mono tracking-tight">{user.email}</div>
                                                                    {(user.facultyId || user.studentId) && (
                                                                        <div className="text-[9px] text-indigo-500 dark:text-indigo-400 font-black font-mono uppercase tracking-widest mt-0.5">
                                                                            #{user.facultyId || user.studentId}
                                                                        </div>
                                                                    )}
                                                                </div>
